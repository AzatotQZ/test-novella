...
label credits:
    scene black with dissolve
    show text "Идея - Regis(Сергей Заварницын) и Hollow48(Николай Денисюк) {p} Код - Regis {p} Арты - Owl {p} Программная среда разработки - RenPy {p} За тестирование спасибо: Almeanay, Belka, Franku {p} Всем спасибо! {p} Все персонажи являются вымышленными и любое совпадение с реально живущим или когда-либо жившими являются оошибочными. {p} The end {p}" at txt_up
    pause 15
    return